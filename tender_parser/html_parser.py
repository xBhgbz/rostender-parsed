...
def parsing_tenders(source_data) -> list:
    ...
    for tender in tenders:
        number = (
            ...
            .get_text(strip=True)
            .replace("\xa0", " ")
            .split("№")[1] if tender.find("span", class_="tender__number") else None
        )
        description = (
            tender.find("a", class_="tender-info__description")
            .get_text(strip=True)
            .replace("\xa0", " ") if tender.find("a", class_="tender-info__description") else None
        )
        date_start = (
            tender.find("span", class_="tender__date-start")
            .get_text(strip=True)
            .replace("\xa0", " ").split(" ")[1] if tender.find("span", class_="tender__date-start") else None
        )
        date_finish = (
            tender.find("span", class_="tender__countdown-text")
            .get_text(strip=True)
            .replace("\xa0", " ")
            .split(")")[1] if tender.find("span", class_="tender__countdown-text") else None
        )
        address = (
            tender.find("div", class_="tender-address")
            .get_text(strip=True)
            .replace("\xa0", " ") if tender.find("div", class_="tender-address") else None
        )
        price = (
            tender.find("div", class_="starting-price__price")
            .get_text(strip=True)
            .replace("\xa0", " ")
            .split(" ₽")[0] if tender.find("div", class_="starting-price__price") else None
        )
        category = (
            tender.find("a", class_="list-branches__link")
            .get_text(strip=True)
            .replace("\xa0", " ") if tender.find("a", class_="list-branches__link") else None
        )
